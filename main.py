...
# fonction qui sert à afficher le joueur qui a le trait
def drawtour(window, gs, is_black):
    turn1 = "Black To Move" if is_black else "White To Move"
    turn2 = "White To Move" if is_black else "Black To Move"
    color1 = p.Color("Black") if is_black else p.Color("White")
    color2 = p.Color("White") if is_black else p.Color("Black")
    color3 = (116, 116, 116) if is_black else (181, 181, 181)
    color4 = (181, 181, 181) if is_black else (116, 116, 116)
    # tour du blanc
    if gs.whiteToMove:

        p.draw.rect(window, color4, p.Rect(0, WIDTH, WIDTH, HEIGHT - 20))
        p.draw.circle(window, color1, (WIDTH // 4, WIDTH + 18), SQ_SIZE // 7)
        FONT = p.font.Font(None, 40)
        TEXT1 = FONT.render(turn1, True, color1)
        textLocation1 = p.Rect(WIDTH // 3, WIDTH + 5, WIDTH - 2, WIDTH - 20)
        window.blit(TEXT1, textLocation1)
    # tour du noir
    else:
        p.draw.rect(window, color3, p.Rect(0, WIDTH, WIDTH, WIDTH - 20))
        p.draw.circle(window, color2, (WIDTH // 4, WIDTH + 18), SQ_SIZE // 7)
        FONT = p.font.Font(None, 40)
        TEXT1 = FONT.render(turn2, True, color2)
        textLocation1 = p.Rect(WIDTH // 3, WIDTH + 5, WIDTH - 2, WIDTH - 20)
        window.blit(TEXT1, textLocation1)
...
def main(mode=True, is_black=False):
    ...
    while run:
        ...
        if not game_over and not humanTurn:
            ...
            # au cas où le AI ne trouve aucun mouvement possible (extremement rare)
            if not AI_move:
